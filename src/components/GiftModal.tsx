...
const paymentOptions: {
...
}[] = [
  {
    id: "paypal",
    label: "PayPal",
    description: "Opens PayPal with the amount ready — then confirm here.",
  },
  {
    id: "revolut",
    label: "Revolut",
...
export function GiftModal({ gift, onClose, onClaimed }: GiftModalProps) {
  ...
  const [paypalInfo, setPaypalInfo] = useState<PayPalPaymentInfo | null>(null);
  ...
  function localPaypalInfo(name: string): PayPalPaymentInfo {
    return {
      url: buildPayPalMeUrl(gift!.price),
      fallbackUrl: buildPayPalXclickUrl(gift!.price, gift!.name, name, gift!.id),
  ...
  async function handleContinue() {
    ...
    try {
      if (paymentMethod === "paypal") {
        const local = localPaypalInfo(name);
        setPaypalInfo(local);
        try {
        ...
        try {
          ...
          if (res.ok) {
            const data = (await res.json()) as Partial<PayPalPaymentInfo>;
            setPaypalInfo({
              url: data.url || local.url,
              fallbackUrl: data.fallbackUrl || local.fallbackUrl,
              email: data.email || local.email,
              username: data.username || local.username,
            });
          }
        } catch {
          /* client-built PayPal link is enough */
        }
        return;
      }
        ...
        <div className="p-8">
          {step === "thanks" ? (
              ...
              {paymentMethod === "paypal" && paypalInfo ? (
                <div className="mt-4 rounded-sm border border-wine/20 bg-white px-5 py-5 text-center">
                  ...
                  <p className="mt-1 font-display text-3xl font-light text-wine">
                    €{gift.price.toFixed(gift.price % 1 === 0 ? 0 : 2)}
                  </p>
                  <p className="mt-2 text-sm text-ink-muted">
                    via PayPal to{" "}
                    <span className="font-medium text-ink">@{paypalInfo.username}</span>
                  </p>
                  <div className="mt-4 flex flex-wrap justify-center gap-2">
                    <CopyChip value={gift.price.toFixed(2)} label="Copy amount" />
                    <CopyChip value={paypalInfo.email} label="Copy PayPal email" />
                  </div>
                  <a
                    href={paypalInfo.url}
                    target="_blank"
                  ...
                  <a
                    href={paypalInfo.fallbackUrl}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="btn-secondary mt-3 inline-flex w-full items-center justify-center"
                  >
                    Alternative PayPal checkout
                  </a>
                  <p className="mt-4 text-xs leading-relaxed text-ink-light">
                    If the first button doesn&apos;t open, use the alternative checkout or send
                    the amount to {paypalInfo.email} in the PayPal app.
                  </p>
                </div>
            ...
            <>
              ...
              {claimError && <p className="mt-4 text-sm text-wine-light">{claimError}</p>}
              ...
              <p className="mt-5 text-center text-xs leading-relaxed text-ink-light">
                {paymentMethod === "paypal"
                  ? "We'll open PayPal with the amount ready. After you pay, come back here and mark the gift as claimed."
